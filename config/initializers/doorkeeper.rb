...
DOORKEEPER_SCOPES = {
  authorization_code: %i[openid profile email phone],
  client_credentials: ['invitations:write', 'users:write', 'challenges:write', 'trades:write']
}.freeze
...
Doorkeeper.configure do
  ...
  resource_owner_from_credentials do
    ...
    request.env['warden'].authenticate!(store: false)
  end
  ...
  admin_authenticator do
    # authorizer = Authorizer.new(Rails.application.config.permitted_domains)
    # if current_user
    #   head :forbidden unless authorizer.permitted?(current_user.email)
    # else
  ...
  #
  # default_scopes
  optional_scopes(*DOORKEEPER_SCOPES.values.flatten)
  ...
  #
  scopes_by_grant_type DOORKEEPER_SCOPES
  ...
  #
  force_ssl_in_redirect_uri Rails.application.config.doorkeeper.force_ssl_in_redirects
  #
...
Doorkeeper::JWT.configure do
  secret_key        Rails.application.config.jwt.keyset.keypair.to_s
  encryption_method Rails.application.config.jwt.encryption_method

  token_payload do |opts|
    resource = User.find_by(id: opts[:resource_owner_id])
    resource = UserClaimsPresenter.new(resource) unless resource.blank?

    Identity::ClaimsBuilder.call(application: opts[:application],
                                 resource: resource,
                                 scopes: opts[:scopes],
                                 expires_in: opts[:expires_in],
                                 created_at: opts[:created_at].to_i,
                                 issuer: Thread.current.fetch(:host, Rails.application.config.jwt.issuer),
                                 token: true)
  end
...
Doorkeeper::OAuth::TokenResponse.prepend Identity::TokenResponse
